$(document).ready(function () {
    ...
    const onChange = async (e) => {
        ...
        if (id === 'location' && $('#location').val() != ''&&$('#location').val().length>0) {
            $('#submit').prop('disabled', true);
            await fetch('https://maps.googleapis.com/maps/api/geocode/json?address=' + value + '&key=' + 'AIzaSyCs1n8Zg39pudq1cOWo17H0BHWTSfFaljo', getAPIObject)
                .then(res => {
                    if (res && res.status === 200)
                        return res.json()
                }
    ...
    const onSubmit = async (e) => {
        ...
        if (validate()) {
            ...
            //important
            await fetch('https://psyched-age-363304.wl.r.appspot.com/getDets?keyWord=' + form.keyWord +'&&category=' + form.category + '&&distance=' + (parseInt(form.distance * 1609.344)) + '&&locationLong=' + form.cords.long  +'&&locationLat=' + form.cords.lat   , getAPIObject)
                .then((response) => {
    ...
    const constructCard = (data) => {
        ...
        let statusLab = 'Status'
        let addressLab = 'Address'
        let phoneLab = 'Phone No'
        let priceLab = 'Price'
        let moreLab = 'More Info'
        let transactionLab = 'Transactions Supported'
        let bHeader = 'bHeader'
        let cBlockLab = 'cBlock'
        let bContent = 'bContent'
        let targetBlank ='blank'
        let cDetailClass = 'cDetails'
        if(price != undefined)
        ...
        let cBlock =  `${(`<div class="cBlock"><p class="bHeader ">${statusLab}</p><div class="bContent"><span class=${ hours && [0] && ['is_open_now'] ? "sGreen" : "sRed"}>${ hours && [0] && ['is_open_now'] ? "Open Now" : "Closed"}</span></div> </div>`)}`
        let locationD = `${getConcat(location.display_address, '   ') && (`<div class=${cBlockLab}><p class="bHeader">${addressLab}</p><div class="bContent"><p class="cDetails">${getConcat(location.display_address, '   ')}</p></div></div>`)}`
        let displayPhone = `${display_phone && (`<div class=${cBlockLab}><p class=${bHeader}> ${phoneLab}</p><div class=${bContent}><p class=${cDetailClass}>${display_phone}</p></div></div>`)}`
        let transactions1 = `${getConcat(transactions, ' | ') && (`<div class=${cBlockLab}><p class="bHeader">${transactionLab}</p><div class="bContent"><p class="cDetails">${getConcat(transactions, ' | ')}</p></div></div>`)}`
        let price11 = `${price1 && (`<div class=${cBlockLab}><p class=${bHeader}>${priceLab} </p><div class=${bContent}><p class=${cDetailClass}>${price1}</p></div> </div>`)}`
        let url1 = `${url && (`<div class=${cBlockLab}><p class=${bHeader}> ${moreLab}</p><div class=${bContent}><a href=${url} target=${targetBlank}>Yelp</a></div></div>`)}</div>`
        if(price1 != undefined)
        ...
        let imgHolder = 'img-hldr'
        let imgContainter = 'image-container'
        cardImages = photos.length > 0 ? (`   <div class="cPhotos">${photos[0] !== undefined && photos[0]?`<div class=${imgContainter}><div class=${imgHolder}><img src=${photos[0]} class=${cardImage}></div><p style='text-align: center;'>${photo1}</p></div>`:(`<div></div>`)}
                ${photos[1] !== undefined && photos[1]? `<div class=${imgContainter}><div class=${imgHolder}><img src=${photos[1]} class=${cardImage}></div><p style='text-align: center;'>${photo2}</p></div>`:(`<div></div>`)}
                ${photos[2] !== undefined && photos[2]  ? `<div class=${imgContainter}><div class=${imgHolder}><img src=${photos[2]} class=${cardImage}></div><p style='text-align: center;'>${photo3}</p></div>`:(`<div></div>`)}</div></div>`) : (`<div></div>`)
    ...
    const buildCard = async (id) => {
        $('#dCard').empty()
        await fetch('https://psyched-age-363304.wl.r.appspot.com/getBusinessDets?id=' + id, getAPIObject).then((response) => {return response.json()}).then((data) => {
            if (data) {
                constructCard(data);
            }})
    }
